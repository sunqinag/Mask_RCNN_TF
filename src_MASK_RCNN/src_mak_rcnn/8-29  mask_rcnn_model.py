...
from tensorflow.python.keras import backend as K  # 载入keras的后端实现
...
class MaskRCNN():  # 内部封装的keras_model为真正模型
    ...
    def build(self, mode):  # 构建Mask R-CNN架构
        ...
        input_image = KL.Input(shape=[512, 512, 3], name="input_image")  # 定义输入节点
        input_image = tf.reshape(input_image, [3, 512, 512, 3])

        if mode == "inference":
            input_anchors = KL.Input(shape=[None, 4], name="input_anchors")  # 将全局的锚点框输入
        elif mode == "training":  # 处理训练接口
            # 由图片和标注生成的关于锚点的值1 -1 0.用于RPN的标签
            input_rpn_match = KL.Input(shape=[4092, 1], name="input_rpn_match", dtype=tf.int32)
            input_rpn_match = tf.reshape(input_rpn_match, [3, 4092, 1])
            # 由图片和标注生成的关于锚点框的偏移量x,y 高宽缩放.用于RPN的标签
            input_rpn_bbox = KL.Input(shape=[256, 4], name="input_rpn_bbox", dtype=tf.float32)
            input_rpn_bbox = tf.reshape(input_rpn_bbox,[3,256, 4])

            # Detection GT (class IDs, bounding boxes, and masks)
            # 1. GT Class IDs (zero padded)
            input_gt_class_ids = KL.Input(
                shape=[100], name="input_gt_class_ids", dtype=tf.int32)
            input_gt_class_ids = tf.reshape(input_gt_class_ids,[3,100])
            # 2. GT Boxes in pixels (zero padded)
            # [batch, MAX_GT_INSTANCES, (y1, x1, y2, x2)] in image coordinates
            input_gt_boxes = KL.Input(
                shape=[100, 4], name="input_gt_boxes", dtype=tf.float32)
            input_gt_boxes=tf.reshape(input_gt_boxes,[3,100,4])
            ...
            if USE_MINI_MASK:
                input_gt_masks = KL.Input(
                    shape=[MINI_MASK_SHAPE[0],
                           MINI_MASK_SHAPE[1], 100],
                    name="input_gt_masks", dtype=bool)
                input_gt_masks = tf.reshape(input_gt_masks,[3,56,56,100])
            else:
                input_gt_masks = KL.Input(shape=[IMAGE_DIM, IMAGE_DIM, 100],
                                          name="input_gt_masks", dtype=bool)
        ...
        rpn_rois = ProposalLayer(proposal_count=proposal_count, nms_threshold=RPN_NMS_THRESHOLD,
        ...
        if mode == "inference":
            ...
            class_loss = KL.Lambda(lambda x: mrcnn_class_loss_graph(self.num_class, self.batch_size, *x),
