...
class UnetEstimator(base_segmentation_estimator.BaseSegmentationEstimator):
    def __init__(self,
                 ...
                 # 卷积特征提取层
                 feature_conv_filter_nums=(16, 32, 64),
                 feature_conv_kernel_sizes=(3, 3, 3),
                 feature_conv_strides=(1, 1, 1),
                 # unet卷积层
                 unet_conv_filter_nums=(16, 32, 64),
                 unet_conv_kernel_sizes=(3, 3, 3),
                 unet_conv_strides=(1, 1, 1),
                 conv_padding='same',
                 ...
                 deconv_filter_nums=(64, 32, 32),
                 deconv_kernel_sizes=(3, 3, 3),
    ...
    def network(self, inputs, is_training):
        ...
        for i in range(len(self.deconv_filter_nums)):
            ...
            if i < (len(self.deconv_filter_nums) - 1):
                # 跨层连接
                for j in range(1, self.conv_num_before_pool):
                    concat_unet_layer = tf.concat(
                        [unet_conv_tensor_list[
                            -self.conv_num_before_pool * i - j],
                         unet_conv_tensor_list[
                             -self.conv_num_before_pool * i - j - 1]], axis=3)
