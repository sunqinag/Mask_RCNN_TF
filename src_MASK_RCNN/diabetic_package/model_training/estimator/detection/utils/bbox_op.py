...
def center_2_corner(center_bbox):
    '''
    center_bbox：
        以左上角和右下角坐标表示的bbox，最后一个维度是4，依次为[center_y,
        center_x, height, width]
    返回值：
        以中心和宽高描述的bbox，shape与center_coord相同，最后一个维度依次是[
        up, left, down, right]
    '''
    if not isinstance(center_bbox, np.ndarray):
        raise ValueError('corner_coord的类型必须是ndarray')
    if center_bbox.shape[-1] != 4:
        raise ValueError('corner_coord的最后一维必须是4')
    center_bbox_shape = center_bbox.shape
    center_bbox = center_bbox.reshape([-1, 4])
    if np.logical_or(center_bbox[:, 2] < 0,
                     center_bbox[:, 3] < 0).any():
        raise ValueError('center_bbox中有bbox的宽度或高度小于0')
    up = (center_bbox[:, 0] - center_bbox[:, 2] / 2).reshape([-1, 1])
    left = (center_bbox[:, 1] - center_bbox[:, 3] / 2).reshape([-1, 1])
    down = (center_bbox[:, 0] + center_bbox[:, 2] / 2).reshape([-1, 1])
    right = (center_bbox[:, 1] + center_bbox[:, 3] / 2).reshape([-1, 1])
    corner_bbox = np.concatenate([up, left, down, right], axis=1)
    return corner_bbox.reshape(center_bbox_shape)
...
def b2t_yolo(bbox,
    ...
    t_w = np.log(b[:, 3] / prior_width).reshape([-1, 1])
    t_h = np.log(b[:, 2] / prior_height).reshape([-1, 1])
