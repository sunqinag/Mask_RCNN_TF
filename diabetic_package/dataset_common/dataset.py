...
class ImageLabelDataSet(ImageLabelDataSetBaseClass):
    ...
    def preprocess_image_map(self, data_dict):
        ...
        image, label = tensorflow_image_processing_map.random_rotate_image_and_label_tf_map(image, label, max_angle=30)
        ...
        image, label = tensorflow_image_processing_map.random_rescale_and_crop_image_and_label_tf_map(
            image, label, crop_width=self.crop_height_width[1], crop_height=self.crop_height_width[0])
        ...
        # #image, label = tensorflow_image_processing_map.transpose_image_and_label_tf_map(image, label)
        # #image, label = tensorflow_image_processing_map.translate_image_and_label_tf_map(image, label, dx=10, dy=-60.0)

        #image, label = tensorflow_image_processing_map.rotate_image_and_label_tf_map(image, label, angle=30)
        # #
        image = tensorflow_image_processing_map.adjust_random_brightness_image_tf_map(image, 0.5)
        # image = tensorflow_image_processing_map.adjust_random_contrast_image_tf_map(image, 0.8, 1.2)
        # # image = tensorflow_image_processing_map.adjust_random_hue_image_tf_map(image, 0.2, rate=1.0)
        # # image = tensorflow_image_processing_map.adjust_random_saturation_image_tf_map(image, 0.0, 0.1)
        # image = tensorflow_image_processing_map.add_random_noise_tf_map(image, 0.5, 10)
        # image = tensorflow_image_processing_map.add_gaussian_noise_tf_map(image, 0.1, 1.0, 1.0)
        #
        # image = tensorflow_image_processing_map.add_salt_and_pepper_noise_pyfunc_tf_map(image, 0.002, 255)
        # image = tensorflow_image_processing_map.adjust_brightness_image_tf_map(image, delta=100, rate=1.0)
        # image = tensorflow_image_processing_map.adjust_contrast_image_tf_map(image, contrast_factor=0.5)
        # image = tensorflow_image_processing_map.adjust_saturation_image_tf_map(image, 0.2)
