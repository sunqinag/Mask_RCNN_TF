...
class Dataset:
    def __init__(self, mode, base_folder, data_reload=True):
        self.mode = mode
        ...
        self.tfrecord_name = str(self.mode) + '.tfrecords'
        if data_reload:
    ...
    def get_single_mask(self, src_label):
        ...
        bbox = np.array(bbox).reshape((MAX_OBJ_NUM, 5)).astype(np.int32)
        ...
        if masks.shape[-1] < MAX_OBJ_NUM:
            ...
            masks = np.concatenate([masks[:,:,:MAX_OBJ_NUM]], axis=2).astype(np.int32)
    ...
    def pareser(self, serialized):
        ...
        class_ids = tf.decode_raw(features['class_ids'],tf.int32)
...
if __name__ == '__main__':
    ...
    dataset = Dataset('val', r'E:\Pycharm_project\mask_rcnn_TF\voc\val')
